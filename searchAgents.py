            ...
            if not hitsWall:
                ...
                if ((newState) in self.corners):
                    ...
                    for i in range(len(cornersGoal)):
                        cornersTemp.append(
                            True) if i == index else cornersTemp.append(cornersGoal[i])
...
def cornersHeuristic(state, problem):
    ...
    # Save currentstate in the list of bools corners
    currentState, cornersGoal = state
    cornersObj = {}  # create dict to relate the cornersGoal to their coords
    unvisitedCorners = []  # create list for unvisited corners
    heuristic = []

    for i in range(len(cornersGoal)):
        # create dict to match coord to bool (1,1):true (already visited)
        cornersObj[corners[i]] = cornersGoal[i]

    for key in cornersObj:                          # loop dict
        if (not cornersObj[key]):
            # if coord is false add it to unvisitedCorners list
            unvisitedCorners.append(key)

    if len(unvisitedCorners) == 0:                  # if list is empty return 0
        return 0

    # for every corner in unvisited corners
    for unvCorners in unvisitedCorners:
        # calc distance between currentState and corner
        heuristic.append(util.manhattanDistance(currentState, unvCorners))
    # return max value in heuristic list
...
def foodHeuristic(state, problem):
    ...
    currentState, cornersGoal = state
    # get all dots
    unvisitedDots = cornersGoal.asList()
    heuristic = []
    # return 0 if list of dots is empty
    if len(unvisitedDots) == 0:
        return 0
    # loop list of unvisitedDots
    for dots in unvisitedDots:
        # calc maze distance between currentState and every fot
        heuristic.append(mazeDistance(
            currentState, dots, problem.startingGameState))
    # return max value of heuristic list
    return max(heuristic)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # check if state in current food list
